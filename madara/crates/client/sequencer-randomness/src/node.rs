...
impl Node {
    ...
    pub async fn drain_retained(&self) -> anyhow::Result<()> {
        ...
        let mut retained: Vec<Felt> = self
            ...
            .collect();
        for mut block in self.backend.internal_preconfirmed_views()?.1 {
    ...
    fn is_candidate(&self, hash: Felt) -> anyhow::Result<bool> {
        Ok(self.backend.internal_preconfirmed_views()?.1.into_iter().any(|mut block| {
            block.refresh_with_candidates();
