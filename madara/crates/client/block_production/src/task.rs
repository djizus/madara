...
impl BlockProductionTask {
    ...
    /// Makes the confirmed tip the durable base for the first new parallel root.
    ///
    /// Both devnet genesis and preconfirmed recovery can write serial blocks after DB startup.
    /// Their diffs are absent from the finalizer's window, even when recovery itself did nothing.
    fn ensure_parallel_merkle_startup_base(&self) -> anyhow::Result<()> {
        if !self.parallel_merkle_enabled {
            return Ok(());
        }

        self.backend
            .reconcile_confirmed_parallel_merkle_state("block_production_startup")
            .context("Establishing the parallel Merkle startup base")
    }
